...
echo "0 2 * * * cd $(pwd) && /usr/local/bin/python scripts/cleanup.py --data-days $DATA_KEEP_DAYS --plots-days $PLOTS_KEEP_DAYS >> logs/cleanup.log 2>&1" > /tmp/cleanup_cron
...
python scripts/cleanup.py --data-days $DATA_KEEP_DAYS --plots-days $PLOTS_KEEP_DAYS >> logs/cleanup.log 2>&1
